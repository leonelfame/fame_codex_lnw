...
REPOSITORY="${CODEX_WEB_GPT_REPOSITORY:-leonelfame/fame_codex_lnw}"
...
VERSION="${VERSION#v}"
if [ -z "$VERSION" ]; then
  echo "Could not resolve the latest Fame Codex release" >&2
  exit 1
fi
case "$VERSION" in
...
ASSET="fame-codex-$VERSION-$PLATFORM-$ARCH.$EXTENSION"
BASE_URL="https://github.com/$REPOSITORY/releases/download/v$VERSION"
TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/fame-codex-launcher.XXXXXX")"
...
if [ "$OS" = "Darwin" ]; then
  ...
  SOURCE_APP="$STAGE_DIR/Fame Codex.app"
  if [ ! -d "$SOURCE_APP" ] || [ ! -x "$SOURCE_APP/Contents/MacOS/Fame Codex" ]; then
  ...
  TARGET_APP="$INSTALL_DIR/Fame Codex.app"
  if pgrep -x "Fame Codex" >/dev/null 2>&1; then
    echo "Quit Fame Codex before updating it" >&2
    exit 1
  fi
  BACKUP_APP="$TEMP_DIR/Fame Codex.previous.app"
...
LIB_DIR="${CODEX_WEB_GPT_LIB_DIR:-$HOME/.local/lib/fame-codex}"
BIN_DIR="${CODEX_WEB_GPT_BIN_DIR:-$HOME/.local/bin}"
TARGET_DIR="$LIB_DIR/$VERSION"
TARGET="$TARGET_DIR/Fame Codex.AppImage"
WRAPPER="$BIN_DIR/fame-codex"
...
if { [ -n "$RUNNING_PID" ] && kill -0 "$RUNNING_PID" 2>/dev/null; } \
  || pgrep -f "Fame Codex\\.AppImage" >/dev/null 2>&1; then
  echo "Quit Fame Codex before updating it" >&2
  exit 1
fi
...
install -m 0644 "$ICON_SOURCE" "$ICON_DIR/fame-codex.png"
DESKTOP_WRAPPER="$(printf '%s' "$WRAPPER" | sed \
  ...
  -e 's/\$/\\$/g' \
  -e 's/%/%%/g')"
cat > "$APPLICATIONS_DIR/fame-codex.desktop" <<EOF
[Desktop Entry]
Type=Application
Version=1.0
Name=Fame Codex
...
StartupWMClass=fame-codex
EOF
chmod 0644 "$APPLICATIONS_DIR/fame-codex.desktop"
