...
const GITHUB_URL = "https://github.com/leonelfame/fame_codex_lnw";
...
if (!IS_DEV_PROFILE) {
  process.env.CODEX_CHATGPT_WEB_LEGACY_HOME = path.join(app.getPath("home"), ".codex-chatgpt-web");
}
app.setName(LAUNCHER_PROFILE.displayName);
if (process.platform === "win32") {
  app.setAppUserModelId(IS_DEV_PROFILE ? "com.leonelfame.famecodex.launcher.dev" : "com.leonelfame.famecodex.launcher");
}
...
const NATIVE_COPY = Object.freeze({
  en: Object.freeze({
    openLauncher: "Open Fame Codex",
    quit: "Quit",
    exportDiagnostics: "Export privacy-safe diagnostics",
    cancel: "Cancel",
    remove: "Remove",
    removeTitle: "Remove Fame Codex",
...
function registerIpc({ logger, stateStore }) {
  ...
  handle("launcher:export-logs", async () => {
    ...
    const result = await dialog.showSaveDialog(mainWindow, {
      ...
      defaultPath: path.join(app.getPath("documents"), `fame-codex-diagnostics-${date}.jsonl`),
...
async function requestQuit() {
  ...
  try {
    ...
    if (activeOperation) {
      throw new Error(`Wait for ${activeOperation} to finish before quitting Fame Codex`);
    }
...
async function start() {
  ...
  if (process.platform === "linux") {
    app.commandLine.appendSwitch("class", IS_DEV_PROFILE ? "fame-codex-dev" : "fame-codex");
  }
    ...
    if (runtime.status === "external" || runtime.status === "needs-setup") {
      const detail = runtime.detail || (
        runtime.status === "external"
          ? "Another process owns the configured Fame Codex runtime"
...
void start().catch((error) => {
  ...
  try {
    dialog.showErrorBox("Fame Codex could not start", message);
  } catch {}
  app.exit(1);
});
