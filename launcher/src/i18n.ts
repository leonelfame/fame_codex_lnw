...
const en = {
  ...
  biggerContextBody: "Warning: keeps small DEV turns as one message, then expands large context to two or three staging messages plus one final execution message. Compaction always uses three stages. Extra requests may increase the chance of rate limits or a temporary cooldown. Disabled by default.",
...
const zh: Record<keyof typeof en, string> = {
  ...
  biggerContextBody: "警告：较小的 DEV 回合仍作为一条消息发送；较大的上下文会扩展为两条或三条暂存消息，再发送一条最终执行消息。压缩始终使用三条暂存消息。额外请求可能提高触发速率限制或临时冷却的概率。默认关闭。",
