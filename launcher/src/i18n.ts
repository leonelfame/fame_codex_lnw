...
const en = {
  ...
  mcpStepTwoBody: "Paste the tunnel ID and runtime key. The tunnel must belong to the same OpenAI account that will use the ChatGPT plugin. The key is imported into private local storage and is never written to launcher logs.",
  tunnelId: "Tunnel ID",
  runtimeKey: "Runtime API key (not Admin key)",
  connect: "Connect harness",
  mcpStepThree: "Attach the ChatGPT connector",
  mcpStepThreeBody: "First enable Developer Mode in ChatGPT settings. Then open ChatGPT Plugins, attach this tunnel, and name it exactly Codex Native. Finally, run the health check.",
...
const zh: Record<keyof typeof en, string> = {
  ...
  mcpStepTwoBody: "粘贴 Tunnel ID 和运行时密钥。Tunnel 必须属于将使用该 ChatGPT 插件的同一 OpenAI 账户。密钥会导入私有本地存储，绝不会写入启动器日志。",
  tunnelId: "Tunnel ID",
  runtimeKey: "Runtime API key（不是 Admin key）",
  connect: "连接 Harness",
  mcpStepThree: "连接 ChatGPT 插件",
  mcpStepThreeBody: "先在 ChatGPT 设置中启用开发者模式。然后打开 ChatGPT Plugins，连接 Tunnel，并将名称设为 Codex Native。最后运行健康检查。",
