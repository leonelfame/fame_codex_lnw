...
const en = {
  ...
  mcpStepOneBody: "Create an OpenAI tunnel, copy its Tunnel ID, and create a regular API key with Tunnels Read + Use (free; the key is required only to run the tunnel).",
  ...
  mcpStepThreeBody: "Before creating the connector, enable Developer Mode in ChatGPT Settings. In ChatGPT Plugins, create a connector, choose Tunnel, select the tunnel you created, set Authentication to None, and name it exactly Codex Native. Then verify the runtime.",
...
const zh: Record<keyof typeof en, string> = {
  ...
  mcpStepOneBody: "创建 OpenAI Tunnel，复制 Tunnel ID，然后创建一个拥有 Tunnels Read + Use 权限的普通 API key（免费；此密钥仅用于运行 Tunnel）。",
  ...
  mcpStepThreeBody: "创建连接器前，先在 ChatGPT 设置中启用开发者模式。然后在 ChatGPT Plugins 中创建连接器，选择 Tunnel，选中刚创建的 Tunnel，将身份验证设为无，并将名称设为 Codex Native。最后验证运行时。",
