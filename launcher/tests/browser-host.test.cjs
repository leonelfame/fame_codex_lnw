...
test("session inspection delegates navigation and capability detection to the shared browser helper", async () => {
  ...
  const fixture = Object.assign(Object.create(BrowserHost.prototype), {
    ...
    getConnectorName: () => "Fame Codex Native2",
    logger: { info() {} },
  ...
  assert.equal(calls[1].appName, "Fame Codex Native2");
...
test("launcher delegates every ChatGPT model and turn operation to the shared browser worker", async () => {
  ...
  assert.equal(helperCall.appName, "Fame Codex Native2");
});
...
test("connector verification is effort-independent and works while the browser surface is hidden", async () => {
  ...
  const result = await BrowserHost.prototype.runConnectorVerification.call(fixture, "Fame Codex Native2");

  assert.deepEqual(result, { ok: true, appName: "Fame Codex Native2" });
...
test("connector verification records the helper failure in launcher diagnostics", async () => {
  ...
  await assert.rejects(
    BrowserHost.prototype.runConnectorVerification.call(fixture, "Fame Codex Native2"),
    failure,
  );
...
test("a later provider round reuses only its exact connector-bound conversation", async () => {
  ...
  const tab = {
    ...
    conversationKey,
    connectorIdentity: "Fame Codex Native2",
    connectorBound: true,
    interactionMode: "automatic",
    helperPid: 111,
    status: "ready",
