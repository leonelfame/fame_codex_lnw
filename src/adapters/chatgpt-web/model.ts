...
export interface ChatGptWebModelMode {
  ...
  displayLabel: "Luna" | "Think" | "Instant" | "Medium" | "High" | "Extra High" | "Pro" | "Astra";
  baseModel: "luna" | "sol" | "astra";
...
export function resolveChatGptWebModelMode(
  ...
  if (modelId === CHATGPT_WEB_ASTRA_MODEL_ID) {
    if (capabilities.astraAvailable !== true) {
      throw new Error("ChatGPT Astra is not available for this account");
    }
    const effort = reasoning ?? "high";
    if (effort !== "high") throw new Error(`ChatGPT Astra effort is not supported: ${effort}`);
    return {
      modelId,
      effort,
      displayLabel: "Astra",
  ...
  switch (effort) {
    case "low":
      return { modelId, effort, displayLabel: "Instant", baseModel: "sol", uiEffortIndex: 0, thinkEnabled: false, localTools: capabilities.localToolsEnabled };
    case "medium":
      return { modelId, effort, displayLabel: "Medium", baseModel: "sol", uiEffortIndex: 1, thinkEnabled: false, localTools: capabilities.localToolsEnabled };
    case "high":
      return { modelId, effort, displayLabel: "High", baseModel: "sol", uiEffortIndex: 2, thinkEnabled: false, localTools: capabilities.localToolsEnabled };
    case "xhigh":
      if (!capabilities.proAvailable) throw new Error("ChatGPT Extra High effort is not available for this account");
      return { modelId, effort, displayLabel: "Extra High", baseModel: "sol", uiEffortIndex: 3, thinkEnabled: false, localTools: capabilities.localToolsEnabled };
    case "max":
      if (!capabilities.proAvailable) throw new Error("ChatGPT Pro effort is not available for this account");
      return { modelId, effort, displayLabel: "Pro", baseModel: "sol", uiEffortIndex: 4, thinkEnabled: false, localTools: capabilities.localToolsEnabled };
    default:
