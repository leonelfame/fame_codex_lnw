...
import { ChatGptCompletionTracker, chatGptImageFilePayloads, chatGptTurnIsComplete } from "../src/adapters/chatgpt-web/browser-worker";
...
describe("ChatGPT outer-native harness v3", () => {
  ...
  test("accepts completed-turn evidence when ChatGPT replaces SPA actions without increasing their count", () => {
    const state = {
      running: false,
      currentText: "new answer",
      initialText: "old answer",
      completionActionVisible: true,
      completionActionCount: 1,
      initialCompletionActionCount: 1,
      sawRunning: true,
    };
    expect(chatGptTurnIsComplete(state)).toBe(true);
    expect(chatGptTurnIsComplete({ ...state, sawRunning: false })).toBe(false);
  });
